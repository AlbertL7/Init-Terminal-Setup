...
if [ "$(id -u)" -ne 0 ]; then
  echo "This script must be run as root. Use sudo or log in as root. Also you should probably install zsh shell before running!"
  exit 1
fi
...
# Ensure the script is run as root
if [ "$(id -u)" -ne 0 ]; then
  echo "This script must be run as root. Use sudo or log in as root. Also, install Zsh before running this script!"
  exit 1
fi
...
# Final message
echo "Setup complete."
echo "Each user's default shell is now set to Zsh."
